...
class ExternalNode {
  ...
  updateNode(node) {
    node.deviceArray.forEach((device) => {
      ...
      device.sensorValue =
        index > -1 ? this.node.deviceArray[index].sensorValue || 0 : 0;
  ...
  getDeviceValue(deviceId) {
    const device = this.node.deviceArray.find(
  ...
  initialize() {
    ...
    this.relaySchedule = new RelaySchedule(
      this.node.deviceArray,
  ...
  publishMqtt() {
    if (this.getTimeDiff() < 30) {
      ...
      this.mqttHandler.publishToTopicWithEncryption(
        this.node,
        this.node.Mqtt.key,
        this.node.Mqtt.topic
      );
    }
  }
  ...
  getRelayIndex(deviceId) {
    return this.node.deviceArray.findIndex(
      (item) => item.deviceID === Number(deviceId)
    );
  }
