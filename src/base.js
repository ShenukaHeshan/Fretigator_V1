...
let relayDriver1 = new relayDrive(
  ...
  function (index) {
    if (CURRENT_STATE > POWER_ON_STATE || CURRENT_STATE != LOCK_STATE) {
      if (typeof AiGrowJson.deviceArray[index] != "undefined")
        formatDeviceData(AiGrowJson.deviceArray[index].deviceID);
    }
  },
  function (err) {
    err.forEach((element) => {
...
let relayDriver2 = new relayDrive(
  ...
  function (index) {
    if (CURRENT_STATE > POWER_ON_STATE || CURRENT_STATE != LOCK_STATE) {
      if (typeof AiGrowJson.deviceArray[index + 8] != "undefined")
        formatDeviceData(AiGrowJson.deviceArray[index + 8].deviceID);
    }
  },
  function (err) {
    err.forEach((element) => {
...
delay(2000).then(() => {
  console.log(
    "Power on " + dateFormat(Date(), "dddd, mmmm dS, yyyy, h:MM:ss TT")
  );
...
setInterval(() => {
  ...
  if (CURRENT_STATE == POWER_ON_STATE) {
      ...
      apiService.getNodeINIAiGrow((response) => {
        ...
        if (AiGrowJson) {
            ...
            function (path, serialData) {
                ...
                if (serialData.R == 1) {
                  if (serialData.S == 2) {
                      ...
                      if (sensor.type_id == 31 && sensor.enable == 1) {
                        if (sensor.receiver_address == serialData.S) {
                          if (levelSensorArray.length > sensor.receiver_index) {
                            sensor.sensorValue = levelSensorCalculate(
                              levelSensorArray[sensor.receiver_index],
                              sensor.sensorADC_WET,
                              sensor.sensorADC_EMPTY,
                              sensor.sensorADC_DRY
                            );
                    ...
                    const externalNode = externalNodes.find(
                      (element) => serialData.S == element.getAddress()
                    );
                    if (externalNode) {
                      ...
                      sensorArray.forEach((sensor) => {
                        if (sensor.type_id == 31 && sensor.enable == 1) {
                          if (sensor.receiver_address == serialData.S) {
                            if (
                              levelSensorArray.length > sensor.receiver_index
                            ) {
                              sensor.sensorValue = levelSensorCalculate(
                                levelSensorArray[sensor.receiver_index],
                                sensor.sensorADC_WET,
                              ...
                              console.warn(
                                "External Node Level sensor Not Installed."
                              );
                            }
                          }
                        }
                  ...
                  mixingTankLevel = getSensorValue(
                    AiGrowJson.current_node_id,
                    mixingTankLevelSensorId
                  );
                  AiGrowJson.freshWaterTankLevel = getSensorValue(
                    AiGrowJson.current_node_id,
                    freshwaterTankLevelSensorId
                  );

                  for (var i = 0; i < AiGrowJson.numberOfLoops; i++) {
                    AiGrowJson.loopSchedules[i].localTankWaterLevel =
                      getSensorValue(
                        AiGrowJson.loopSchedules[i]
                          .listenTo_FM_LS_CurrentNodeID,
                        AiGrowJson.loopSchedules[i].localTankWaterLevelDeviceID
                      );
                  }
                }
              }
              ...
              if (serialData.hasOwnProperty("FM")) {
                ...
                if (serialData.R == 1) {
                      ...
                      if (sensor.type_id == 25 && sensor.enable == 1) {
                        if (sensor.receiver_address == serialData.S) {
                          if (flowMetersArray.length > sensor.receiver_index) {
                            sensor.sensorValue =
                              flowMetersArray[sensor.receiver_index];
                            sensor.sensorDetect = 1;
                            if (sensor.sensorDisplayLCD) {
                              CF.SensorDataLog(sensor);
                            }
                    ...
                    if (externalNode) {
                      ...
                      sensorArray.forEach((sensor) => {
                        if (sensor.type_id == 25 && sensor.enable == 1) {
                          if (sensor.receiver_address == serialData.S) {
                            if (
                              flowMetersArray.length > sensor.receiver_index
                            ) {
                              sensor.sensorValue =
                                flowMetersArray[sensor.receiver_index];
                              sensor.sensorDetect = 1;
                              ...
                              console.warn(
                                "External Node Flow Meter sensor Not Installed."
                              );
                            }
                          }
                        }
                  ...
                  mixingFlowMeterValue = getSensorValue(
                    AiGrowJson.current_node_id,
                    mixingFlowMeterSensorId
                  );

                  for (var i = 0; i < AiGrowJson.numberOfLoops; i++) {
                    AiGrowJson.loopSchedules[i].loopInFlowMeterValue =
                      getSensorValue(
                        AiGrowJson.loopSchedules[i]
                          .listenTo_FM_LS_CurrentNodeID,
                        AiGrowJson.loopSchedules[i].loopInFlowMeterDeviceID
                      );
                    AiGrowJson.loopSchedules[i].loopOutFlowMeterValue =
                      getSensorValue(
                        AiGrowJson.loopSchedules[i]
                          .listenTo_FM_LS_CurrentNodeID,
                        AiGrowJson.loopSchedules[i].loopOutFlowMeterDeviceID
                      );
                  }
                }
              }
              ...
              if (
                serialData.hasOwnProperty("EC") &&
                serialData.hasOwnProperty("PH")
              ) {
                ...
                AiGrowJson.sensorArray.forEach((sensor) => {
                  if (
                    sensor.receiver_address == serialData.S &&
                    sensor.enable == 1
                  ) {
                  ...
                  if (externalNode) {
                    ...
                    nodeDeviceArray.forEach((device, index) => {
                      if (index < serialData.ex_fb.length) {
                        ...
                        let additional_data =
                          device.additional_data || "0,0,0,0";
                    ...
                    AiGrowJson.loopSchedules.forEach((loop) => {
                      if (loop.loopCurrentNodeID == externalNode.getId()) {
                        loop.loopOutDeviceValue = externalNode.getDeviceValue(
                          loop.loopOutDeviceID
                        );
                        loop.loopDrainPumpRelayValue =
                          externalNode.getDeviceValue(
                            loop.loopDrainPumpRelayDeviceID
                          );
                      }
                    });
                  }
          ...
          relaySchedule = new RelaySchedule(
            AiGrowJson.deviceArray,
            (deviceID, scheduleIndex, relaySchedule, duration) => {
              const relayIndex = getRelayIndex(deviceID);
              if (relayIndex == -1) return;

              const device = AiGrowJson.deviceArray[relayIndex];
              if (
                device.userInvolveValue == "auto" &&
                device.remoteEnable == 1
              ) {
            ...
            function (data) {
              if (data.targetCurrentNodeID == AiGrowJson.current_node_id) {
                if (data.command == "USERSET") {
                  ...
                  if (
                    data.value == "on" &&
                    AiGrowJson.deviceArray[relayIndex].remoteEnable == 1
                  ) {
                    ...
                    updateRelayDevice(data.value, TURNOFF);
                    relaySchedule.reschedule(
                      AiGrowJson.deviceArray[relayIndex]
                    );
                  }
                ...
                const externalNode = externalNodes.find(
                  (element) => data.targetCurrentNodeID == element.getId()
                );
                if (externalNode) {
                  if (data.command == "USERSET") {
    ...
    if (sensorHandler.isSerialActive()) {
      if (externalNodes.every((node) => node.isSerialActive())) {
        ...
        if (
          ...
          ![ERROR_STATE, SAFETY_CHECK].includes(status.STATE)
        ) {
          if (
            status.mode !== undefined &&
            status.currentLoop != null &&
            status.recipeId != null
          ) {
            fertigationStart({
            ...
            if (element.preStageTime == longestOnRelay.preStageTime) {
              if (CURRENT_STATE == PRE_FERTIGATE_STATE) {
                relayOnValueChecker(element, TURNON, function (relayFeedBack) {
                  CF.Log(
                    `Mode: ${element.mode}, Node ID: ${element.currentNodeId}, Device ID: ${element.deviceId}, Value: ${TURNON}, Feedback: ${relayFeedBack}`
                  );
                });
              }
            }
            ...
            customRelaySettings.forEach((element) => {
              if ([1].indexOf(element.mode) > -1) {
                relayOnValueChecker(element, TURNOFF, function (relayFeedBack) {
                  CF.Log(
                    `Mode: ${element.mode}, Node ID: ${element.currentNodeId}, Device ID: ${element.deviceId}, Value: ${TURNOFF}, Feedback: ${relayFeedBack}`
                  );
                });
              }
        ...
        customRelaySettings.forEach((element) => {
          if ([3, 4].indexOf(element.mode) > -1) {
            if (CURRENT_STATE == PRE_FERTIGATE_STATE) {
              relayOnValueChecker(element, TURNON, function (relayFeedBack) {
                CF.Log(
                  `Mode: ${element.mode}, Node ID: ${element.currentNodeId}, Device ID: ${element.deviceId}, Feedback: ${relayFeedBack}`
                );
              });
            }
          }
    ...
    const recipeList =
      AiGrowJson.loopSchedules[currentLoopIndex].loopMultiRecipe.find(
        (recipe) => recipe.id === currentRecipeId
      )?.recipe_list || [];
    const isTimeOrRate = recipeList.some((dosingStep) =>
      ["TIME", "RATE"].includes(dosingStep.mode)
    );
                          ...
                          const interval = setInterval(() => {
                            ...
                            if (getCurrentLevel() >= targetLevel) {
                              ...
                              relayOnValueChecker(
                                ...
                                TURNOFF,
                                function (freshWaterPumpRelayState) {
                                  freshWaterPumpStatus =
                                    freshWaterPumpRelayState;
                                  if (freshWaterPumpRelayState) {
                                    relayOnValueChecker(
                                      mixingSolenoidRelay,
                                      TURNOFF,
                                      function (mixingSolenoidRelayState) {
                                        if (mixingSolenoidRelayState) {
                                          relayOnValueChecker(
                                            freshWaterSolenoidRelay,
                                            TURNOFF,
                                            function (freshWaterSolenoidRelay) {
                              ...
                              console.log(
                                `Tank Level - ${getCurrentLevel()} Fill Until ${targetLevel}`
                              );
                            }
                          }, 100);
                        }
    ...
    const recipeList = AiGrowJson.loopSchedules[
      currentLoopIndex
    ].loopMultiRecipe.find(
      (recipe) => recipe.id === currentRecipeId
    )?.recipe_list;
    if (recipeList?.some((dosingStep) => dosingStep.mode === "RATE")) {
      if (!dosingBusy && !rateModeDone) {
        const stepList = recipeList?.filter(
          (dosingStep) => dosingStep.mode === "RATE"
        );
    ...
    if (recipeList?.some((dosingStep) => dosingStep.mode === "TIME")) {
      if (!dosingBusy && !timeModeDone) {
        const stepList = recipeList?.filter(
          (dosingStep) => dosingStep.mode === "TIME"
        );
        CF.LogCurrentRecipe(null, stepList);
    ...
    if (recipeList?.some((dosingStep) => dosingStep.mode === "RATIO")) {
      if (!dosingBusy && !ratioModeDone) {
        const stepList = recipeList?.filter(
          (dosingStep) => dosingStep.mode === "RATIO"
        );
        const finalEC = Number(
          AiGrowJson.loopSchedules[currentLoopIndex].loopMultiRecipe.find(
            (obj) => obj.id == currentRecipeId
          )?.final_ec
        );
    ...
    if (recipeList?.some((dosingStep) => dosingStep.mode === "EC")) {
      if (!dosingBusy && !ecModeDone) {
        const stepList = recipeList?.filter(
          (dosingStep) => dosingStep.mode === "EC"
        );
        const sortedStepList = stepList.sort(
      ...
      if (
        (EC_ERROR || ecValueStable5SecFlag) &&
        AiGrowJson.minorVersion == VENTURI_TYPE
      ) {
                          ...
                          if (freshWaterPumpRelayState) {
                            const interval = setInterval(() => {
                              ...
                              if (getCurrentLevel() >= setTankLevel) {
                                ...
                                console.log(
                                  `Remaining Tank Level - ${getCurrentLevel()} Fill Until ${setTankLevel}..`
                                );
                              }
                            }, 100);
                          }
                              ...
                              loopOutputValveEnable(
                                ...
                                function (relayFeedback) {
                                  if (relayFeedback) {
                                    if (CURRENT_STATE == N_STATE) {
                                      if (
                                        CURRENT_MODE == "FERTIGATE" ||
                                        CURRENT_MODE == "FERTIGATEWITHDRAIN"
                                      ) {
    ...
    if (!drainTankFlag) {
      drainTankFlag = true;
      if (
        AiGrowJson.loopSchedules[currentLoopIndex].localTankWaterLevel >
        AiGrowJson.loopSchedules[currentLoopIndex].localTankDrainLevel
      ) {
        relayOnValueChecker(
          drainSolenoidRelay,
          TURNON,
          function (drainSolenoidState) {
                ...
                function (relayFeedback) {
                  if (relayFeedback) {
                    const interval = setInterval(() => {
                      ...
                      if (
                        AiGrowJson.loopSchedules[currentLoopIndex]
                          .localTankWaterLevel <=
                        AiGrowJson.loopSchedules[currentLoopIndex]
                          .localTankDrainLevel
                      ) {
                        ...
                        console.log(
                          `Loop Tank Level ${AiGrowJson.loopSchedules[currentLoopIndex].localTankWaterLevel} Drain Until ${AiGrowJson.loopSchedules[currentLoopIndex].localTankDrainLevel}`
                        );
                      }
                    }, 100);
                  }
    ...
    if (!emptyMixingTankFlag) {
      ...
      if (getCurrentLevel() > mixingTankDrainLevel) {
        ...
        relayOnValueChecker(
          drainSolenoidRelay,
          TURNON,
          function (drainSolenoidRelayState) {
                  ...
                  if (mixingPumpRelayState) {
                    const interval = setInterval(() => {
                      ...
                      if (getCurrentLevel() <= mixingTankDrainLevel) {
                        ...
                        console.log(
                          `Mixing Tank Level ${getCurrentLevel()} Drain Until ${mixingTankDrainLevel}`
                        );
                      }
                    }, 500);
                  }
                      ...
                      function (mixingPumpRelayState) {
                        if (mixingPumpRelayState) {
                          const interval = setInterval(() => {
                            ...
                            if (
                              mixingTankLevel <=
                              setTankLevel * EC_REDUCE_DRAIN_PERCENTAGE
                            ) {
                              sensorHandler.disableSensorCheck(
                                mixingTankLevelSensorId
                              );
                          ...
                          if (freshWaterPumpRelayState) {
                            const interval = setInterval(() => {
                              ...
                              if (getCurrentLevel() >= setTankLevel) {
                                ...
                                console.log(
                                  `Filling water Until ${setTankLevel}..`
                                );
                              }
                            }, 100);
                          }
  ...
  if (AiGrowJson) {
    ...
    AiGrowJson.status[0].flowMeterLocalTankIN = currentLoopIndex
      ? AiGrowJson.loopSchedules[currentLoopIndex].loopInFlowMeterValue
      : 0;
    AiGrowJson.status[0].flowMeterLocalTankOUT = currentLoopIndex
      ? AiGrowJson.loopSchedules[currentLoopIndex].loopOutFlowMeterValue
      : 0;
    AiGrowJson.status[0].flowMeterFreshWater = 0;
    AiGrowJson.status[0].flowMeterMixingLoop = 0;
    AiGrowJson.status[0].flowMeterDrain = 0;
    AiGrowJson.status[0].localTankWaterLevel = currentLoopIndex
      ? AiGrowJson.loopSchedules[currentLoopIndex].localTankWaterLevel
      : 0;
    ...
    AiGrowJson.loopSchedules.forEach((loop) => {
      if (loop.loopCurrentNodeID == AiGrowJson.current_node_id) {
        loop.loopOutDeviceValue = getRelayExternalValueByDeviceId(
          loop.loopOutDeviceID
        );
        loop.loopDrainPumpRelayValue = getRelayExternalValueByDeviceId(
          loop.loopDrainPumpRelayDeviceID
        );
      }
    });
...
function rateControlledDosing(rateList) {
  ...
  rateList.forEach((step) => {
    const tank = AiGrowJson.dosingTanks.find(
      (tank) => tank.tank_id == step.tank_id
    );

    step.timeout = Math.round(
      (1000 * getCurrentLevel() * step.rate_value) / DOSING_PUMP_ML_PER_SECOND
    );
    ...
    relayOnValueChecker(device, TURNON, (relayFeedBack) => {
      CF.Log(
        `Mode: RATE, Node ID: ${device.currentNodeId}, Device ID: ${device.deviceId}, Value: ${TURNON}, Feedback: ${relayFeedBack}`
      );
    });
    ...
    const interval = setTimeout(() => {
      relayOnValueChecker(device, TURNOFF, (relayFeedBack) => {
        CF.Log(
          `Mode: RATE, Node ID: ${device.currentNodeId}, Device ID: ${device.deviceId}, Value: ${TURNOFF}, Feedback: ${relayFeedBack}`
        );
      });
...
function timeControlledDosing(timeSteps) {
  ...
  timeSteps.forEach((step) => {
    ...
    relayOnValueChecker(device, TURNON, (relayFeedBack) => {
      CF.Log(
        `Mode: TIME, Node ID: ${device.currentNodeId}, Device ID: ${device.deviceId}, Value: ${TURNON}, Feedback: ${relayFeedBack}`
      );
    });
    ...
    const interval = setTimeout(() => {
      relayOnValueChecker(device, TURNOFF, (relayFeedBack) => {
        CF.Log(
          `Mode: TIME, Node ID: ${device.currentNodeId}, Device ID: ${device.deviceId}, Value: ${TURNOFF}, Feedback: ${relayFeedBack}`
        );
      });
...
function ratioControlledDosing(ratioSteps, setEC) {
  ...
  if (EC_VALUE < setEC) {
    ...
    const adjustedDifference =
      Math.max(minEffectiveDifference, ecDifference) * 2; // Adjusted EC difference for scaling
    ...
    const dosingDuration =
      Math.round(adjustedDifference * dynamicMixingFactor * 2) * 1000; // Total dosing time based on adjustments
    ...
    ratioSteps.forEach((step) => {
      ...
      relayOnValueChecker(device, TURNON, (relayFeedBack) => {
        CF.Log(
          `Mode: RATIO, Node ID: ${device.currentNodeId}, Device ID: ${device.deviceId}, Value: ${TURNON}, Feedback: ${relayFeedBack}`
        );
      });
      ...
      const interval = setTimeout(() => {
        relayOnValueChecker(device, TURNOFF, (relayFeedBack) => {
          CF.Log(
            `Mode: RATIO, Node ID: ${device.currentNodeId}, Device ID: ${device.deviceId}, Value: ${TURNOFF}, Feedback: ${relayFeedBack}`
          );
        });
        ...
        if (ratioSteps.every((step) => step.isCompleted)) {
          ...
          const delayTime =
            currentLevel < 100 ? REDUCED_DELAY_TIME : STANDARD_DELAY_TIME;
...
function ecControlledDosing(ecSteps) {
  ...
  if (EC_VALUE < targetEC) {
    const tank = AiGrowJson.dosingTanks.find(
      (tank) => tank.tank_id === currentStep.tank_id
    );
    if (!tank) {
    ...
    if (AiGrowJson.minorVersion == VENTURI_TYPE) {
      ...
      relayOnValueChecker(device, TURNON, (relayFeedback) => {
        CF.Log(
          `EC Dosing ON: Node ID: ${device.currentNodeId}, Device ID: ${device.deviceId}, Feedback: ${relayFeedback}`
        );
      });
      ...
      const dosingTimeout = setTimeout(() => {
        relayOnValueChecker(device, TURNOFF, (relayFeedback) => {
          CF.Log(
            `EC Dosing OFF: Node ID: ${device.currentNodeId}, Device ID: ${device.deviceId}, Feedback: ${relayFeedback}`
          );
        });
...
function phControlledDosing(step) {
  ...
  if (AiGrowJson.minorVersion == VENTURI_TYPE) {
    if (PH_VALUE >= adjustedMinPh && PH_VALUE <= adjustedMaxPh) {
      ...
      const acidTankId = AiGrowJson.dosingTanks.find(
        (item) => item.chemical_mode == "ACID"
      )?.tank_id;
      const baseTankId = AiGrowJson.dosingTanks.find(
        (item) => item.chemical_mode == "BASE"
      )?.tank_id;
  ...
  if (!dosingBusy) {
    ...
    if (value === TURNON) {
      relayOnValueChecker(device, TURNON, function (relayFeedback) {
        CF.Log(
          `Mode: BASE, Node ID: ${device.currentNodeId}, Device ID: ${device.deviceId}, Value: ${TURNON}, Feedback: ${relayFeedback}`
        );
      });
        ...
        function (remainingTime, interval) {
          ...
          if (remainingTime < dosingTime / PH_TURNOFF_THRESHOLD_FACTOR) {
            relayOnValueChecker(device, TURNOFF, function (relayFeedback) {
              CF.Log(
                `Mode: BASE 1, Node ID: ${device.currentNodeId}, Device ID: ${device.deviceId}, Value: ${TURNOFF}, Feedback: ${relayFeedback}`
              );
            });
          }
        }
      ...
      relayOnValueChecker(device, TURNOFF, function (relayFeedback) {
        CF.Log(
          `Mode: BASE 2, Node ID: ${device.currentNodeId}, Device ID: ${device.deviceId}, Value: ${TURNOFF}, Feedback: ${relayFeedback}`
        );
      });
    }
  }
...
function dosingPumpAcid(value, different) {
  ...
  if (!dosingBusy) {
    ...
    if (value === TURNON) {
      relayOnValueChecker(device, TURNON, function (relayFeedback) {
        CF.Log(
          `Mode: ACID, Node ID: ${device.currentNodeId}, Device ID: ${device.deviceId}, Value: ${TURNON}, Feedback: ${relayFeedback}`
        );
      });
      ...
      secondsTimer(
        ...
        function (remainingTime, interval) {
          ...
          if (remainingTime < dosingTime / PH_TURNOFF_THRESHOLD_FACTOR) {
            relayOnValueChecker(device, TURNOFF, function (relayFeedback) {
              CF.Log(
                `Mode: ACID 1, Node ID: ${device.currentNodeId}, Device ID: ${device.deviceId}, Value: ${TURNOFF}, Feedback: ${relayFeedback}`
              );
            });
          }
        }
      ...
      relayOnValueChecker(device, TURNOFF, function (relayFeedback) {
        CF.Log(
          `Mode: ACID 2, Node ID: ${device.currentNodeId}, Device ID: ${device.deviceId}, Value: ${TURNOFF}, Feedback: ${relayFeedback}`
        );
      });
    }
  }
...
function handleError(errorCode, data = {}) {
  ...
  switch (errorCode) {
    ...
    case SERIAL_ERROR:
      message =
        "Not received serial data for over 30 seconds. Please contact AiGrow.";
      logError(message, data);
    ...
    case DEVICE_ERROR:
      ...
      if (currentNodeId == AiGrowJson.current_node_id) {
        foundDevice = AiGrowJson.deviceArray.find(
...
function relayOnValueChecker(
  ...
  if (device.currentNodeId == AiGrowJson.current_node_id) {
    ...
    const externalNode = externalNodes.find(
      (node) => node.getId() == device.currentNodeId
    );
    if (externalNode) {
...
function getRelayIndex(deviceId) {
  return AiGrowJson.deviceArray.findIndex(
    (item) => item.deviceID == Number(deviceId)
  );
}
...
function ValueBind() {
  AiGrowJson.deviceArray.forEach((device, index) => {
    ...
    AiGrowJson.deviceArray[index].sensorValue =
      1 - relayDriver.get().relayExternalValue[tempIndex];
    AiGrowJson.deviceArray[index].additional_data =
      relayDriver.get().relayValueShadow[tempIndex] +
      "," +
      "0" +
      "," +
      relayDriver.get().relayOnByShadow[tempIndex] +
      "," +
      relayDriver.get().relayExternalValue[tempIndex];
    AiGrowJson.deviceArray[index].lastFetch = dateFormat(
      new Date(),
...
function updateAiGrowJson(newData, existingData) {
  ...
  newData.deviceArray.forEach((newDevice, index) => {
    if (newDevice.userInvolveValue == "on" && newDevice.remoteEnable == 1) {
      ...
      newDevice.userInvolveValue == "off" &&
      newDevice.remoteEnable == 1
    ) {
      relaySwitchExternalWrapper(index, TURNOFF);
    } else if (
      newDevice.userInvolveValue == "auto" &&
      newDevice.remoteEnable == 1
    ) {
      relaySwitchExternalWrapper(index, TURNOFF);
  ...
  newData.sensorArray.forEach((newSensor) => {
    const sensor = existingData.sensorArray.find(
      (item) => item.deviceID == Number(newSensor.deviceID)
    );
    if (sensor) {
  ...
  newData.loopSchedules.forEach((newLoop) => {
    const loop = existingData.loopSchedules.find(
      (item) => item.loop_id == Number(newLoop.loop_id)
    );
    if (loop) {
  ...
  newData.dataBus.forEach((busData) => {
    const node = externalNodes.find(
      (item) => item.getId() == busData.current_node_id
    );
    if (node) {
...
function getLongestPreStageRelaySettings() {
  return customRelaySettings
    .filter((element) => [1, 2, 6].includes(element.mode))
    .reduce(
      (max, element) =>
        max == null || element.preStageTime > max.preStageTime ? element : max,
      null
    );
}
...
function getLongestPostStageRelaySettings() {
  return customRelaySettings
    .filter((element) => [4, 5, 6].includes(element.mode))
    .reduce(
      (max, element) =>
        max == null || element.postStageTime > max.postStageTime
          ? element
          : max,
      null
    );
...
function fertigationStart({
  ...
  function initializeFertigationCycleData() {
    ...
    fertigationCycleData.start = dateFormat(
      new Date(),
    ...
    fertigationCycleData.recipe =
      loop.loopMultiRecipe.find((recipe) => recipe.id == currentRecipeId)
        ?.recipe_list || [];
  ...
  function addToRemainingCycle() {
    ...
    if (!addToRemainingCycle.interval) {
      addToRemainingCycle.interval = setInterval(() => {
        if (
          CURRENT_STATE == IDLE_STATE &&
          remainingFertigationCycle.length > 0
        ) {
  ...
  if (CURRENT_STATE == IDLE_STATE) {
    ...
    setTankLevel =
      tankLevel ?? AiGrowJson.loopSchedules[currentLoopIndex].localTankSetLevel;
    ...
    if (fertigationMode === "FERTIGATE" || fertigationMode === "FRESHWATER") {
      ...
      ["DRAIN", "FERTIGATEWITHDRAIN", "FRESHWATERWITHDRAIN"].includes(
        fertigationMode
      )
    ) {
      if (AiGrowJson.majorVersion == MIXING_TYPE) {
...
function fertigationEnd() {
  customRelaySettings.forEach((device) => {
    relayOnValueChecker(device, TURNOFF, (relayFeedback) => {
      CF.Log(
        `Fertigation End Node ID: ${device.currentNodeId}, Device ID: ${device.deviceId}, Feedback: ${relayFeedback}`
      );
    });
  });
...
function cycleEndAPICall(isComplete) {
  ...
  fertigationCycleData.endTankLevel =
    AiGrowJson.loopSchedules[currentLoopIndex].localTankWaterLevel;
  fertigationCycleData.endTime = dateFormat(
    new Date(),
    "yyyy/mm/dd HH:MM:ss TT"
  );
...
function stopFertigation() {
  try {
    ...
    customRelaySettings.forEach((device) => {
      relayOnValueChecker(device, TURNOFF, (relayFeedback) => {
        CF.Log(
          `Safety Stopped Node ID: ${device.currentNodeId}, Device ID: ${device.deviceId}, Feedback: ${relayFeedback}`
        );
      });
    });

    const pumps = AiGrowJson.deviceArray.filter((device) =>
      device.type.includes("Pump")
    );
    const others = AiGrowJson.deviceArray.filter(
      (device) => !device.type.includes("Pump")
    );
...
function getSensorValue(currentNode_id, deviceId) {
  if (currentNode_id == AiGrowJson.current_node_id) {
    const device = AiGrowJson.sensorArray.find(
      (item) => item.deviceID == deviceId
    );
    return device ? device.sensorValue : 0;
  } else {
    const externalNode = ExternalNodes.find(
      (element) => element.getId() == currentNode_id
    );
    if (externalNode) {
      const device = externalNode
        .getNode()
        .sensorArray.find((item) => item.deviceID == deviceId);
      return device ? device.sensorValue : 0;
    }
  }
...
function fertigateSchedule() {
  AiGrowJson.loopSchedules.forEach((loop, loopIndex) => {
    ...
    loop.MultiRecipeSchedulelist.forEach((loopSchedule, scheduleIndex) => {
      ...
      const fertigationMode = loopSchedule.startWithDrain
        ? "FERTIGATEWITHDRAIN"
        : "FERTIGATE";
      ...
      scheduleList[scheduleIndex] = schedule.scheduleJob(cronExpression, () => {
        console.log(
          colors.black.bgGreen(
            `Fertigation triggered by schedule. Name: ${loopSchedule.ScheduleName} Mode: ${fertigationMode}, cron: ${cronExpression}`
          )
        );
        fertigationStart({
          loopIndex,
      ...
      console.log(
        `    Next run at: ${scheduleList[scheduleIndex].nextInvocation()}`
      );
    ...
    if (loop.drainSchedule) {
      loopDrainScheduleList[loopIndex] = schedule.scheduleJob(
        loop.drainSchedule,
        () => {
          fertigationStart({
            loopIndex,
...
function updateRecipe(recipe) {
  ...
  console.log(
    `Recipe Update Received for Loop ID - ${recipe.loopId} | Date - ${currentDate}`
  );
  ...
  switch (recipe.Command) {
    ...
    case "DELETE_RECIPE":
      loopSchedule.loopMultiRecipe = loopSchedule.loopMultiRecipe.filter(
        (existingRecipe) => existingRecipe.id !== recipe.recipeId
      );
    ...
    case "UPDATE_RECIPE_STEPS":
      const recipeToUpdate = loopSchedule.loopMultiRecipe.find(
        (existingRecipe) => existingRecipe.id === recipe.recipeId
      );
      ...
      if (recipeToUpdate) {
        ...
        console.error(
          `Recipe with ID ${recipe.recipeId} not found in loop ${recipe.loopId}.`
        );
      }
